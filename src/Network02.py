# -*- coding: utf-8 -*-
"""
Created on Thu Mar  2 10:51:08 2017
...
class Network:
    ...
    def backpropMatrix(self,training_data):
        nabla_b=[np.zeros(b.shape) for b in self.biases]
        nabla_w=[np.zeros(w.shape) for w in self.weights]
        X= [np.reshape(x, (784, 1)) for x in training_data[0]]
        X=np.matrix(np.array(X)).T
        Y=np.matrix(np.array([vectorized_result(y) for y in training_data[1]])).T
        a=X
        Acti_layer=[X]
        Zc=[]
        for b,w in zip(self.biases,self.weights):
...
net.SGD(list(training_data), 30, 10, 0.5, list(test_data))
'''


training_data, validation_data, test_data = mnist_loader.load_data()
net=Network([784, 30, 10])
Zc,Acti_layer=net.backpropMatrix(training_data)
